...
/* Tunables */
static const int sync_read_expire = HZ / 10;	/* max time before a read sync is submitted. */
static const int sync_write_expire = HZ / 3;	/* max time before a write sync is submitted. */
static const int async_read_expire = HZ / 5;	/* ditto for read async, these limits are SOFT! */
...
static unsigned int is_screen_on = 1;
...
static struct notifier_block maple_lcd_notif;

static int maple_lcd_notifier_callback(struct notifier_block *this,
...
maple_dispatch_requests(struct request_queue *q, int force)
{
	...
	if (!rq) {
		...
		if (is_screen_on == 1 && mdata->starved >= mdata->writes_starved)
			data_dir = WRITE;
		else if (is_screen_on == 0 && mdata->starved >= 1)
			data_dir = WRITE;
...
static void *maple_init_queue(struct request_queue *q)//, struct elevator_type *e)
{
...
{
	maple_lcd_notif.notifier_call = maple_lcd_notifier_callback;
	if (lcd_register_client(&maple_lcd_notif) != 0) {
...
{
	lcd_unregister_client(&maple_lcd_notif);
	/* Unregister elevator */
