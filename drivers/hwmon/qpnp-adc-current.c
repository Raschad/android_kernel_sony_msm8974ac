				...
				struct qpnp_iadc_result *result)
{
	...
	if (dt_index >= iadc->max_channels_available) {
		...
		dt_index = 0;
		while (((enum qpnp_iadc_channels) iadc->adc->adc_channels[dt_index].channel_num
				!= channel) && (dt_index < iadc->max_channels_available)) {
			pr_err("dump: adc_channel[%d].channel_num = %d",
					dt_index, iadc->adc->adc_channels[dt_index].channel_num);
			dt_index++;
		}
		goto old_again1;
	}
...
static int __devinit qpnp_iadc_probe(struct spmi_device *spmi)
{
	...
	dev_err(&spmi->dev, "iadc->max_channels_available %d\n",iadc->max_channels_available);
...
fail:
	...
	dev_err(&spmi->dev, "End error probe...\n");
