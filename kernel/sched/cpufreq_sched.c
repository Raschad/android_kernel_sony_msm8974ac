...
static void cpufreq_sched_try_driver_target(struct cpufreq_policy_n *policy,
					    unsigned int freq)
{
...
static int cpufreq_sched_thread(void *data)
{
	...
	struct cpufreq_policy_n *policy;
	struct gov_data *gd;
	unsigned int new_request = 0;
	unsigned int last_request = 0;
	int ret;

	policy = (struct cpufreq_policy_n *) data;
...
static int cpufreq_sched_policy_init(struct cpufreq_policy_n *policy)
{
	struct gov_data *gd;
	int cpu;
	int rc;
...
static int cpufreq_sched_policy_exit(struct cpufreq_policy_n *policy)
{
	unsigned int count;
	struct gov_data *gd = policy->governor_data;
...
static int cpufreq_sched_start(struct cpufreq_policy_n *policy)
{
	int cpu;
...
static void cpufreq_sched_limits(struct cpufreq_policy_n *policy)
{
...
static int cpufreq_sched_stop(struct cpufreq_policy_n *policy)
{
	int cpu;
...
static int cpufreq_sched_setup(struct cpufreq_policy_n *policy,
			       unsigned int event)
{
	switch (event) {
